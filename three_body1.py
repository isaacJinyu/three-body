...
v = np.zeros((n, 3, 3)) # 第一维是时间，第二维是星体，第三维是速度分量
r[0, 0, :] = r1
r[0, 1, :] = r2
r[0, 2, :] = r3
v[0, 0, :] = v1
...
def animate(i):
    ...
    x1, y1, z1 = r[i, 0, :]  # 获取第一个点的位置
    ax.set_xlim(x1 - 1e12, x1 + 1e12)  # 设置 x 轴的范围
    ax.set_ylim(y1 - 1e12, y1 + 1e12)  # 设置 y 轴的范围
    ax.set_zlim(z1 - 1e12, z1 + 1e12)  # 设置 z 轴的范围
    ax.view_init(elev=20, azim=i / 5)  # 设置视角

    # 清除旧的标签
    for text in ax.texts:
        text.remove()

    ax.text(r[i, 0, 0], r[i, 0, 1], r[i, 0, 2], f'v1 = {np.linalg.norm(v[i, 0, :]):.1f} m/s', color='r', fontsize=10, horizontalalignment='left', verticalalignment='top')
    ax.text(r[i, 1, 0], r[i, 1, 1], r[i, 1, 2], f'v2 = {np.linalg.norm(v[i, 1, :]):.1f} m/s', color='b', fontsize=10, horizontalalignment='left', verticalalignment='top')
    ax.text(r[i, 2, 0], r[i, 2, 1], r[i, 2, 2], f'v3 = {np.linalg.norm(v[i, 2, :]):.1f} m/s', color='g', fontsize=10, horizontalalignment='left', verticalalignment='top')
    # 返回结果
